% this is the second step to generate Fig. 1e - calculating SI based on the
% LFP features within NREM
...
for session_list = 1:length(animal_info)
    ...
    LFP_features_NREM_trans = LFP_features_all';
    features_py = py.numpy.array(LFP_features_NREM_trans(:).');
    features_py  = py.numpy.reshape(features_py ,int64(size(LFP_features_NREM_trans')));
    ...
    if savedata
        save(fullfile(animaldir,[prefix,'.rawStructureIndex-','.mat']),'LFP_features_all','pupil_label','Structure_index','shuf_SI_vals');
    end
    close all
end
