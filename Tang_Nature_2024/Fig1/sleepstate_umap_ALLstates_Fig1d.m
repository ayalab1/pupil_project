...
Sleep_state = [SleepState.idx.timestamps,SleepState.idx.states];  %1 WAKE, 3 NREM, 5 REM
sleepss_id = find(Sleep_state(:,1) <= epochtimes(2) & Sleep_state(:,1) >= epochtimes(1));
Sleep_state_ep = Sleep_state(sleepss_id,:);

for state = 1:3 %WAKE, NREM, REM
...
% bz_PowerSpectrumSlope.m from AYALab neurocode
...
% Remove transients
zFFTspec = NormToInt(spec.amp,'modZ'); % NormToInt.m from AYALab neurocode
totz = NormToInt(abs(sum(zFFTspec,2)),'modZ');
...
%% run UMAP across all sleep states, use umapAndEppFileExchange (4.1) toolbox
% when running the first time, save template
% LFP_features_umap = run_umap(LFP_features(:,2:end),'n_components',
% 2,'min_dist',0.6,'n_neighbors',20,'metric','cosine','save_template_file', 'SleepStates_UMAP_PPP7Day12Ep4_template.mat'); 

% use the template for replicability
LFP_features_umap = run_umap(LFP_features(:,2:end),'n_components', 2,'min_dist',0.6,'n_neighbors',20,'metric','cosine','template_file', 'SleepStates_UMAP_PPP7Day12Ep4_template.mat'); 
