function [gain,shGain,predictions,er,sh,w] = GLMgain_control(source,target,removed_dim,varargin)
...
%
%    removed_dim    variable/dimension to be ablated
%    =========================================================================
%     Properties        Values
%    -------------------------------------------------------------------------
...
% Modified by Wenbo Tang 2024 to generate ablated GLM models (Ref. Tseng et al. 2022)
%
...
for unit = 1:nUnits,
    ...
    for set = 1:nSets,
        lastwarn('');
        [weights,dev] = glmfit(source(setID~=set,:),target(setID~=set,unit),dist,'link',link);
        AIC = dev + 2*length(weights);
        
        if strcmp(lastwarn,'Iteration limit reached.') % if the algorhithm failed to converge
            continue
        end
        weights(removed_dim + 1) = 0; % ablate a coef
