...
OptoCtrl = 1; % Opto session = 1; Ctrl Session = 0
...
pupil_d_fit = locsmooth(behavior.onlinetracking.Area,30,6);
...
%% plot the trace from the whole session
figure,
subplot(311)
plot(pupil_trace(:,1),pupil_trace(:,2))
hold on
plot(pupil_trace(:,1),pupil_d_fit)
plot(pupil_trace(:,1),behavior.onlinetracking.pupil_sws)
...
xlim([6770,6850]);
%% find the example segment
validid = find(pulses.timestamps(:,1) >= 6770 & pulses.timestamps(:,1)<=6850);
pulselist = pulses.timestamps(validid,:);
[ wavAvg_pulse, lfpAvg_pulse ] = wb_eventWavelet (Ripple_lfp, pulselist(:,1));
%% plot the example segment
fig = figure;
plot_number = 0;
for i = 1:length(pulselist(:,1))
    plot_number = plot_number +1;
    supersubplot(fig, 3, 3, plot_number);
    current_wave = lfpAvg_pulse.all_trace(:,i);
    plot(lfpAvg_pulse.timestamps,current_wave)
end
