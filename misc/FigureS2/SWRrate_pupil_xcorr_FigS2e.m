...
animal_info = [{'HYC2'},{2},{2};...
    ...
    {'PPP8'},{8},{3}];
%% define parameters
min_duration = 30; %s, minimal duration for NREM episodes
%% gather data
...
for session_list = 1:length(animal_info)
    ...
    SWSid = find(Sleep_state_ep(:,2) == 3);% 3 for NREM, change it to 1 for WAKE
    SWS_vec_ep(SWSid) = 1; 
    SWSlist = vec2list(SWS_vec_ep, Sleep_state_ep(:,1));
    SWSdur = SWSlist(:,2) - SWSlist(:,1);
    SWSlist = SWSlist(find(SWSdur > min_duration),:);
    ...
    for seg = 1:length(SWSlist(:,1))
        ...
        pupil_d_fit_sws = [pupil_d_fit_sws, pupil_d_fit(segid)];
        segid = find(t >= currentseg(1) & t <= currentseg(2));
        R_sws = [R_sws, R(segid)];
    end

    % zscore
    R = (R-nanmean(R_sws))/nanstd(R_sws);
    pupil_d_fit = (pupil_d_fit-nanmean(pupil_d_fit_sws))/nanstd(pupil_d_fit_sws);
    ...
    % calculated xcorr for each NREM episodes
    for seg = 1:length(SWSlist(:,1))
        currentseg = SWSlist(seg,:);
        segid = find(pupil_time >= currentseg(1) & pupil_time <= currentseg(2));
        if ~isempty(segid)
