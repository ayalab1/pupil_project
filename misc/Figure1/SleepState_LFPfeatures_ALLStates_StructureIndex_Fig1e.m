...
for session_list = 1:length(animal_info)
    ...
    for ep = eps
        ...
        load(fullfile(animaldir,[prefix,'.SleepStateLFPfeatures_ALLStates-','EP',num2str(ep),'.LFP.mat']));
        LFP_features_all = [LFP_features_all;LFP_features];
        pupil_label_all = [pupil_label_all;pupil_label];
    end
    
    % UMAP
    LFP_features_umap = run_umap(LFP_features_all(:,2:end),'n_components', 2,'min_dist',0.6,'n_neighbors',20,'metric','cosine');

    %% calculate structure index
    LFP_features_umap_trans = LFP_features_umap';
    features_py = py.numpy.array(LFP_features_umap_trans(:).');
    features_py  = py.numpy.reshape(features_py ,int64(size(LFP_features_umap_trans')));

    label_py = py.numpy.array(pupil_label_all(:).');

    result = si_pymod.compute_structure_index(pyargs(...
        'data',features_py,'label',label_py,...
        'num_shuffles',int16(nshuffles),'n_neighbors',int16(20)));
    ...
    if savedata
        save(fullfile(animaldir,[prefix,'.StructureIndex-ALLStatesN20','.mat']),'LFP_features_umap','pupil_label','Structure_index','shuf_SI_vals');
    end
    close all
end
