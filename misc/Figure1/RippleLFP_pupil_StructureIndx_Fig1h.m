...
for session_list = 1:length(animal_info)
    ...
    % UMAP
    LFP_features_umap = run_umap(lfp_ripples_all,'n_components', 2,'min_dist',0.6,'n_neighbors',100,'metric','cosine');
    ...
    if savedata
        save(fullfile(animaldir,[prefix,'.StructureIndex-RipplePupil-NoRS','.mat']),'LFP_features_umap','pupil_label_all','Structure_index','shuf_SI_vals');
    end
    close all
end
