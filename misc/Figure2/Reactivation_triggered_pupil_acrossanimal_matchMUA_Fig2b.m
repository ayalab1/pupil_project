    ...
    if ~isempty(SWSlist)
        ...
        for i = 1:length(riplist_ep)
            ...
            if length(find(~isnan(pupil_d))) > 10
                ...
                reactivation_trigger_pupil = [reactivation_trigger_pupil;riplist_RSZ_ep(i)];
...
reactivation_triggerZ_pupil = reactivation_trigger_pupil(validid);
...
samplenum = 4800;  % randomly take half of the data
...
for i = 1:5
    ...
    reactivation_tile_resample{i} = reactivation_tile{i}(indices);
end
reactivation_tile_resample{6} = reactivation_tile{6}(randid(1:samplenum));
