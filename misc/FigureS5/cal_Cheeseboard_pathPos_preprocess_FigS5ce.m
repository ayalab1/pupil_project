...
OptoCtrl = 1; % 1 Opto or small pupil, 0 Ctrl or large pupil
animalname = 'PPP12';
day = 10;
...
for tr = 1:traintrial_num
    ...
    diffposx = diff(posx_smooth); % get running direction

    posy_smooth = filtfilt(posfilt,1,trial_pos(:,2));
    diffposy = diff(posy_smooth); % get running direction
    ...
    pathlength(tr) = nansum(pos_diff); % path length
