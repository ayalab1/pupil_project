...
%%
% dir = '/Volumes/AYALab_Data/Pupil_data/';
...
reactivationRecent_trigger_pupil = [];
reactivationRemote_trigger_pupil = [];
for session_list = 1:length(animal_info)
    ...
    if  strcmp(animalname,'SNCG09')|| strcmp(animalname,'PVR4')|| strcmp(animalname,'PVR5')
    ...
    if ~isempty(SWSlist)
        ...
        for rip = 1:length(validid)
            ripid1 = find(strength_recent(:,1) >= riplist_ep(rip,1) & strength_recent(:,1) <= riplist_ep(rip,2));
            ripid2 = find(strength_remote(:,1) >= riplist_ep(rip,1) & strength_remote(:,1) <= riplist_ep(rip,2));
            if ~isempty(ripid1) && ~isempty(ripid2)
                riplist_RS_recent = [riplist_RS_recent;nanmean(strength_recent(ripid1,2))];
                riplist_RS_remote = [riplist_RS_remote;nanmean(strength_remote(ripid2,2))];
        ...
        for i = 1:length(riplist_ep)
            ...
            if length(find(~isnan(pupil_d))) > 10
                ...
                reactivationRecent_trigger_pupil = [reactivationRecent_trigger_pupil;riplist_RSZ_recent_ep(i)];
                reactivationRemote_trigger_pupil = [reactivationRemote_trigger_pupil;riplist_RSZ_remote_ep(i)];
            end
        end
    end
end
...
reactivationRecent_triggerZ_pupil = reactivationRecent_trigger_pupil(validid);
reactivationRemote_triggerZ_pupil = reactivationRemote_trigger_pupil(validid);
% zscore
reactivationRecent_triggerZ_pupil = (reactivationRecent_triggerZ_pupil - nanmean(reactivationRecent_triggerZ_pupil))./nanstd(reactivationRecent_triggerZ_pupil);
reactivationRemote_triggerZ_pupil = (reactivationRemote_triggerZ_pupil - nanmean(reactivationRemote_triggerZ_pupil))./nanstd(reactivationRemote_triggerZ_pupil);
ripple_triggerZ_pupil_mean = nanmean(ripple_triggerZ_pupil(:,301:end),2);

reactivation_Index_pupil = ( reactivationRemote_trigger_pupil(validid) -  reactivationRecent_trigger_pupil(validid));
