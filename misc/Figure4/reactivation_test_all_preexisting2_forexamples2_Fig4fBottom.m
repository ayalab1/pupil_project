...
eps_postSleep = [3,5]; 
eps_preSleep = 1;
PLA_component = 13;%6 or 13,example assembly number
%% define parameter
...
PLA_component = plastic_Components(PLA_component);
...
for tile = 1:3 % small, middle, large
    ...
    temp_post_PLA = PETH(strength(:,[1 PLA_component+1]),postRipples,'durations',durations,'nBins',nBins);
  

    % zscore by baseline distribution
    baseline = temp_post_PLA(:,baseline_ids);
    baseline_mean = nanmean(baseline(:));
    baseline_std = nanstd(baseline(:));
    temp_post_PLA = (temp_post_PLA - baseline_mean)./baseline_std;
    
    PLA_tile{tile} = temp_post_PLA;
    clear temp_post_PLA
end
...
RS_post_PLA = PETH(strength(:,[1 PLA_component+1]),postRipples,'durations',durations,'nBins',nBins);
% zscore by baseline distribution
baseline = RS_post_PLA(:,baseline_ids);
baseline_mean = nanmean(baseline(:));
baseline_std = nanstd(baseline(:));
RS_post_PLA = (RS_post_PLA - baseline_mean)./baseline_std;

RS_pre_PLA = PETH(strength(:,[1 PLA_component+1]),preRipples,'durations',durations,'nBins',nBins);
% zscore by baseline distribution
baseline = RS_pre_PLA(:,baseline_ids);
baseline_mean = nanmean(baseline(:));
baseline_std = nanstd(baseline(:));
RS_pre_PLA = (RS_pre_PLA - baseline_mean)./baseline_std;
