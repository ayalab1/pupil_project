...
velfilter = 1; 
win = -30:0.1:30;
freq_threshold = []; % ripple frequency threshold in Hz, 120 Hz
%% gather data
...
for session_list = 1:length(animal_info)
    ...
    load(fullfile(animaldir,[prefix,'.optoripples.events.mat'])) % ripple
    ...
    % zscore ripple amp and duration
    riplist_ampZ_ep = (riplist_amp - nanmean(riplist_amp))./nanstd(riplist_amp);
    riplist_durZ_ep = (riplist_dur - nanmean(riplist_dur))./nanstd(riplist_dur);
