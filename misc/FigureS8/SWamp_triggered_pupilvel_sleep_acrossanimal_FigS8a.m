...
% data list, sesssions without a SW channel are excluded
...
for session_list = 1:length(animal_info)
    ...
    riplist_all = [ripples.timestamps(:,1),ripples.timestamps(:,1) + ripples.duration,ripples.amplitudeSw]; % [start, end, amplitude]
    %% restrict to NREM
    if ~isempty(SWSlist)
